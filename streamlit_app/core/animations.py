...
def confetti(trigger: bool = True) -> None:
    ...
    Pure CSS implementation — Streamlit strips <script> tags, so we use
    CSS keyframe animations on absolutely-positioned elements instead.
    """
    if not trigger:
        return
    # Generate 40 confetti pieces with varied colours, positions, and timing
    colours = ["#6366f1","#8b5cf6","#22c55e","#f59e0b","#3b82f6","#ec4899","#fafafa","#a78bfa"]
    pieces = ""
    import random as _rnd
    _rnd.seed(42)  # deterministic so reruns look the same
    for i in range(40):
        left   = _rnd.randint(2, 98)
        delay  = _rnd.uniform(0, 1.6)
        dur    = _rnd.uniform(1.8, 3.2)
        size   = _rnd.randint(6, 12)
        color  = colours[i % len(colours)]
        rotate = _rnd.randint(0, 360)
        pieces += (
            f'<div style="position:absolute;left:{left}%;top:-20px;'
            f'width:{size}px;height:{size//2}px;background:{color};'
            f'border-radius:2px;'
            f'animation:confettiFall {dur:.1f}s {delay:.2f}s ease-in forwards;'
            f'transform:rotate({rotate}deg)"></div>'
        )
    st.markdown(f"""
<style>
...
<div style="position:fixed;inset:0;z-index:9998;pointer-events:none;overflow:hidden">
{pieces}
</div>
""", unsafe_allow_html=True)
