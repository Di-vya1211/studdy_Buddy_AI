...
import {
  ...
  Brain, Layers, BarChart2, FileText, ChevronRight, GraduationCap,
} from "lucide-react";
...
const TABS: { id: AppTab; label: string; icon: React.ElementType; color: string; desc: string }[] = [
  { id: "upload",     label: "Study Material", icon: Upload,        color: "text-blue-400",    desc: "Upload or paste text" },
...
export default function LearnPage() {
  ...
  /** Text entered in the "Paste / Type Text" mode */
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">Upload a file or paste text to get started</p>
        </div>
      </div>
            ...
            >
              {activeTab === "upload" && (
                <FileUpload
                  ...
                  onTextInput={setPastedText}
                  pastedText={pastedText}
                />
              )}
              {activeTab === "explain" && (
                <ExplainModule docId={activeDocId} topicOverride={!activeDocId ? pastedText : undefined} />
              )}
              {activeTab === "quiz" && (
                <QuizGame docId={activeDocId} topicOverride={!activeDocId ? pastedText : undefined} />
              )}
              {activeTab === "planner" && (
                <RevisionPlanner docId={activeDocId} syllabusOverride={!activeDocId ? pastedText : undefined} />
              )}
              {activeTab === "doubt" && (
                <DoubtSolver docId={activeDocId} />
              )}
              {activeTab === "feynman" && (
                <FeynmanMode docId={activeDocId} conceptOverride={!activeDocId ? pastedText : undefined} />
              )}
              {activeTab === "flashcards" && (
                <Flashcards docId={activeDocId} topicOverride={!activeDocId ? pastedText : undefined} />
              )}
